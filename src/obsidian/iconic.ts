 ...
 * us when the user sets an icon. Callers redraw on their own schedule, and
 * `addIconicReloadAction` below gives a view the button for exactly this.
 */
 ...
 * the button doing nothing at all. The repaint is the plugin's, which redraws every
 * view that draws icons rather than only the one clicked — the cache they read is
 * shared, so an icon that went stale went stale in all of them at once. Asking for
 * the plugin rather than a callback is what keeps that true: a view handing over
 * its own redraw would leave the others showing an icon nothing can invalidate.
 */
export function addIconicReloadAction(
  view: ItemView,
  plugin: { refreshIconic: () => void },
): HTMLElement {
  return view.addAction("refresh-cw", "Reload icons", () => {
    forgetIconicIcons();
    plugin.refreshIconic();
  });
}
